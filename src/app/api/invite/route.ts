...
const RATE_LIMITS = {
  ORGANIZATION: { MAX_EMAILS: 25, WINDOW_HOURS: 24 }, // Primary limit: 25 emails per day per organization
  IP: { MAX_EMAILS: 50, WINDOW_HOURS: 24 }, // IP protection: 50 emails per day per IP
  BATCH: { MAX_EMAILS: 25 }, // Batch protection: max 25 emails per request
} as const;
...
async function verifyUserAndOrganization(
  ...
  try {
    ...
    // First check if they're in the current organization
    const isInCurrentOrg = userData.organizationId === organizationId;

    // Also check if they have a role in this organization through orgRoles
    const hasOrgRole = userData.orgRoles?.some(
      (role: any) => role.orgId === organizationId
    );

    if (!isInCurrentOrg && !hasOrgRole) {
      return {
    ...
    // Get user's role in this organization from their orgRoles array
    const userOrgRole = userData.orgRoles?.find(
      (role: any) => role.orgId === organizationId
    );

    if (!userOrgRole) {
      return {
        error: "User role not found in organization",
        status: 403,
      };
    }
    ...
    if (!allowedRoles.includes(userOrgRole.orgRole)) {
      return {
    ...
    return { userId, userRole: userOrgRole.orgRole };
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    // Check rate limits
    const [orgRateLimit, ipRateLimit] = await Promise.all([
