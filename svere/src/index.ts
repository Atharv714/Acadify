...
async function getClassroomCourses(userId: string, accessToken: string) {
...
async function getCourseWork(userId: string, courseId: string, accessToken: string) {
  oauth2Client.setCredentials({ access_token: accessToken });
  const classroom = google.classroom({ version: "v1", auth: oauth2Client });
  ...
  const submissionMap = new Map(
    (submissions.data.studentSubmissions || []).map((s) => [
      s.courseWorkId,
      s.state,
    ])
  );
  ...
  for (const cw of courseWork) {
    ...
    const state =
      submissionState === "TURNED_IN" || submissionState === "RETURNED"
        ? "Completed"
...
async function getGmailMessages(userId: string, accessToken: string, query = "", maxResults = 25) {
  oauth2Client.setCredentials({ access_token: accessToken });
...
async function streamAlerts(userId: string) {
  const alerts: any[] = [];
  ...
  try {
    ...
    switch (name) {
      case "get_classroom_courses":
        const courses = await getClassroomCourses(args.userId as string, args.accessToken as string);
        return {
      ...
      case "get_dashboard_summary":
        const summary = await getDashboardSummary(args.userId as string);
        return {
          content: [{ type: "text", text: JSON.stringify(summary, null, 2) }],
        };

      case "stream_alerts":
        const alerts = await streamAlerts(args.userId as string);
        return {
